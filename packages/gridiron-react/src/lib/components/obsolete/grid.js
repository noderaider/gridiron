...
export default function grid (deps, defaults) {
   ...
   */
  const SCROLL_POSITION_CHANGE_REASONS =  { OBSERVED: 'observed'
                                          , REQUESTED: 'requested'
                                          }
  ...
  const templatesShape =
    { GridHeader: PropTypes.func.isRequired
    , ColumnHeader: PropTypes.func.isRequired
    ...
    , ColumnFooter: PropTypes.func.isRequired
    , GridFooter: PropTypes.func.isRequired
    }

  const defaultTemplates =
    { GridHeader: ({ context, ...props }) => <div {...props} />
    , ColumnHeader: ({ context, ...props }) => <div {...props} />
    , Row: ({ context, rowID, ...props }) => (
        <div>
          <div><Pre>{rowID}</Pre></div>
          <div {...props} />
        </div>
      )
    , Cell: ({ context, rowID, cellID, cellDatum, ...props }) => <div {...props}><Pre>{{ [cellID]: cellDatum }}</Pre></div>
...
/*
    ...
    , init() {
        ...
        this._getEstimatedColumnSize = props => typeof props.columnWidth === 'number' ? props.columnWidth : props.estimatedColumnSize
        this._getEstimatedRowSize = props => typeof props.rowHeight === 'number' ? props.rowHeight : props.estimatedRowSize
        ...
        this._invokeOnGridRenderedHelper = () => {
          ...
          this._onGridRenderedMemoizer(
            ...
            , indices:  { columnOverscanStartIndex: this._columnStartIndex
       ...
       */
        ...
        this._setScrollPosition = ({ scrollLeft, scrollTop }) => {
          const newState = { scrollPositionChangeReason: SCROLL_POSITION_CHANGE_REASONS.REQUESTED }
        ...
        this._wrapCellClassNameGetter = className => this._wrapPropertyGetter(className)
        this._wrapCellStyleGetter = style => this._wrapPropertyGetter(style)
        this._wrapSizeGetter = size => this._wrapPropertyGetter(size)

        this._wrapPropertyGetter = value => {
          return value instanceof Function
            ? value
            : () => value
        }

        this._updateScrollLeftForScrollToColumn = (props = this.props, state = this.state) => {
          ...
          if (scrollToColumn >= 0 && columnCount > 0) {
            ...
            if (scrollLeft !== calculatedScrollLeft)
              this._setScrollPosition({ scrollLeft: calculatedScrollLeft })
          }
        }

        this._updateScrollTopForScrollToRow = (props = this.props, state = this.state) => {
        ...
        this._onScroll = event => {
          ...
          if (
            ...
            this._setNextState( { isScrolling: true
                                , scrollLeft
        ...
        this._columnWidthGetter = this._wrapSizeGetter(this.props.columnWidth)
        this._rowHeightGetter = this._wrapSizeGetter(this.props.rowHeight)
        ...
        */
         ...
         */
        this.recomputeGridSize = ({ columnIndex = 0
                                  , rowIndex = 0
                                  } = {}) => {
    ...
    , componentDidMount () {
        ...
        this._invokeOnScrollMemoizer( { scrollLeft: scrollLeft || 0
    ...
    , componentDidUpdate (prevProps, prevState) {
        ...
        */
        ...
        /*
        updateScrollIndexHelper({ cellSizeAndPositionManager: this._columnSizeAndPositionManager
        ...
        updateScrollIndexHelper({ cellSizeAndPositionManager: this._rowSizeAndPositionManager
    ...
    , componentWillUpdate (nextProps, nextState) {
      /*
        if(nextProps.columnCount === 0 && nextState.scrollLeft !== 0 || nextProps.rowCount === 0 && nextState.scrollTop !== 0)
          this._setScrollPosition({ scrollLeft: 0, scrollTop: 0 })
        else if(nextProps.scrollLeft !== this.props.scrollLeft || nextProps.scrollTop !== this.props.scrollTop)
          this._setScrollPosition({ scrollLeft: nextProps.scrollLeft, scrollTop: nextProps.scrollTop })

        this._columnWidthGetter = this._wrapSizeGetter(nextProps.columnWidth)
        this._rowHeightGetter = this._wrapSizeGetter(nextProps.rowHeight)

        this._columnSizeAndPositionManager.configure( { cellCount: nextProps.columnCount
                                                      , estimatedCellSize: this._getEstimatedColumnSize(nextProps)
                                                      } )
        this._rowSizeAndPositionManager.configure({ cellCount: nextProps.rowCount
    ...
    , render () {
        ...
        //const gridHeader = cols.entrySeq().map(([]))

        const gridBody = data.entrySeq().map(([ rowID, context ]) => {
          const rowProps = { rowID, context }
          return (
            <templates.Row key={rowID} {...rowProps}>
              {templates.RowHeader ? (
                <templates.RowHeader key={`${rowID}-header`} {...rowProps} />
              ) : null}
              {(
                context.get('cellData').entrySeq().map(([ cellID, cellDatum ]) => {
                  const cellProps = { cellID, cellDatum }
                  return (
                    <templates.Cell key={cellID} {...rowProps} {...cellProps} />
                  )
                })
              )}
              {templates.RowFooter? (
                <templates.RowFooter key={`${rowID}-footer`} {...rowProps} />
              ) : null}
            </templates.Row>
          )
        })
...
//VIRTUALIZED STUFF NOT IMPORTANT FOR GRIDS WITH PAGING
